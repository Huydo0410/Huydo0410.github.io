...
{if(typeof value=='undefined'||value==null)value='';value=$.trim(value.replace(/'/g,"\\'").replace(/"/g,'\\"'));return($.isNumeric(value))?value*1:'"'+value+'"';};$.fbuilder['parseVal']=function(value,thousandSeparator,decimalSymbol)
...
$('#fieldlist'+opt.identifier).find(".pbPrevious,.pbNext").bind("keyup",function(evt){if(evt.which==13||evt.which==32)$(this).click();}).bind("click",{'identifier':opt.identifier},function(evt){var identifier=evt.data.identifier;if(($(this).hasClass("pbPrevious"))||(($(this).hasClass("pbNext"))&&$(this).closest("form").valid()))
...
form.attr('data-evalequations',this.evalequations).attr('data-evalequationsevent',this.evalequationsevent).attr('autocomplete',((this.autocomplete)?'on':'off')).find('input,select').blur(function(){try{$(this).valid();}catch(e){};});}});var theForm,ffunct={toShow:{},toHide:{},hiddenByContainer:{},getItem:function(name)
{for(var i in items)
...
{var ts=this.thousandSeparator,cs=((cs=$.trim(this.centSeparator))!=='')?cs:'.',v=$.fbuilder.parseVal(value,ts,cs),parts=[],counter=0,str='',sign='';if(!isNaN(v))
{if(v<0)sign='-';v=ABS(v);parts=v.toString().split(".");for(var i=parts[0].length-1;i>=0;i--)
{counter++;str=parts[0][i]+str;if(counter%3==0&&i!=0)str=ts+str;}
parts[0]=str;if(parts[1])
{if(parts[1].length==1)parts[1]+='0';}
else parts[1]='00';return this.currencySymbol+sign+parts.join(cs)+this.currencyText;}
else
...
{var f=el.id.match(/_\d+$/),esc=$.fbuilder.escape_symbol,r;e=$.fbuilder['forms'][f[0]].getItem(el.name);r=new RegExp('^\\s*('+esc(e.currencySymbol)+')?\\s*\\-?\\d+('+esc(e.thousandSeparator)+'\\d{3})*('+e.centSeparator+'\\d+)?\\s*('+esc(e.currencyText)+')?\\s*$','i');return this.optional(el)||r.test(v)||$.isNumeric(v);},$.validator.messages['currency']);$.validator.methods.min=function(v,e,p)
{var f=e.id.match(/_\d+$/);v=$.fbuilder['forms'][f[0]].getItem(e.name).val();return this.optional(e)||v>=p;};$.validator.methods.max=function(v,e,p)
{var f=e.id.match(/_\d+$/);v=$.fbuilder['forms'][f[0]].getItem(e.name).val();return this.optional(e)||v<=p;};}},val:function()
...
{var ts=this.thousandSeparator,ds=((ds=$.trim(this.decimalSymbol))!=='')?ds:'.',v=$.fbuilder.parseVal(value,ts,ds),s='',counter=0,str='',parts=[];if(!isNaN(v))
...
{var f=el.id.match(/_\d+$/),esc=$.fbuilder.escape_symbol,r;e=$.fbuilder['forms'][f[0]].getItem(el.name);r=new RegExp('^\\s*\\-?\\d+('+esc(e.thousandSeparator)+'\\d{3})*('+e.decimalSymbol+'\\d+)?\\s*$','i');return this.optional(el)||r.test(v)||$.isNumeric(v);};$.validator.methods.min=function(v,e,p)
{var f=e.id.match(/_\d+$/);v=$.fbuilder['forms'][f[0]].getItem(e.name).val();return this.optional(e)||v>=p;};$.validator.methods.max=function(v,e,p)
{var f=e.id.match(/_\d+$/);v=$.fbuilder['forms'][f[0]].getItem(e.name).val();return this.optional(e)||v<=p;};}},val:function()
{var e=$('[id="'+this.name+'"]:not(.ignore)');if(e.length)
...
{return this.optional(e)||((minDate==null||date>=minDate)&&(maxDate==null||date<=maxDate)&&item.validateDate($(e).datepicker('getDate'),item.working_dates,item.invalidDates)[0]&&item.validateTime(e,item));}
return true;}
catch(er)
{return false;}};if(!('dateddmmyyyy'in $.validator.methods))$.validator.addMethod("dateddmmyyyy",validator);if(!('datemmddyyyy'in $.validator.methods))$.validator.addMethod("datemmddyyyy",validator);me.setDefaultDate();me.setDefaultTime();me.setEvents();me.set_date_time();},val:function()
{var e=$('[id="'+this.name+'"]:not(.ignore)');if(e.length)
...
delete toShow[dep];}
if(typeof toShow[dep]=='undefined')
{$('#'+dep).closest('.fields').hide();$('[id*="'+dep+'"]:not(.ignore)').addClass('ignore');toHide[dep]={};}}
else
{delete toHide[dep];if(typeof toShow[dep]=='undefined')
toShow[dep]={'ref':{}};toShow[dep]['ref'][me.name+'_'+i]=1;if(!(dep in hiddenByContainer))
{$('#'+dep).closest('.fields').show();$('[id*="'+dep+'"].ignore').removeClass('ignore');}}
if($.inArray(dep,result)==-1)result.push(dep);}}});}
catch(e){}
return result;},val:function()
{var e=$('[id="'+this.name+'"]:checked:not(.ignore)'),v,me=this;if(!me.merge)v=[];if(e.length)
{e.each(function(){var t=$.fbuilder[(me.merge)?'parseVal':'parseValStr'](this.value);if(!$.isNumeric(t))t=t.replace(/^"/,'').replace(/"$/,'');if(me.merge)v=(v)?v+t:t;else v.push(t);});}
...
return'<div class="fields '+me.csslayout+' cff-checkbox-field" id="field'+me.form_identifier+'-'+me.index+'"><div class="dfield">'+'<div class="one_column"><label><input name="'+me.name+'" id="'+me.name+'" class="field required" value="'+$.fbuilder.htmlEncode(me.value)+'" vt="'+$.fbuilder.htmlEncode((/^\s*$/.test(me.value))?me.title:me.value)+'" type="checkbox" /> <span>'+
